...
// matches <foo>...</foo> extracts content
    ...
    function parseSheet(data) {
        ...
        //init hyperlinks
        var hprlnk = {};
        data.match(/<hyperlinks>([^\u2603]*)<\/hyperlinks>/m)[1].split("<").forEach(function (part) {
            if (part.match(/hyperlink ref=.*\/>/)) {
                part = part.replace("/>", "");
                var link = {};
                var id;
    ...
    function parseHyperlink(data) {
        if (!data) return data;
        var linkList = {};
        data.split("<").forEach(function (part) {
            if (part.match(/Relationship Id=.*\/>/)) {
                part = part.replace("/>", "");
                var link = {};
                var id;
                part.split(" ").forEach(function (relationshipItem) {
        ...
        if (!props.Worksheets) {
            ...
            for (i = 0; i != props.Worksheets; ++i) {
                try { /* TODO: remove these guards */
                    ...
                    hyperlinks[props.SheetNames[i]] = parseHyperlink(getdata(getzipfile(zip, 'xl/worksheets/_rels/sheet' + (i + 1) + '.xml.rels')));
                } catch (e) {
                }
            }
        }
